...
def writer():#collecotorがリストにしたデータをcsvファイルに保存する。  
    with open("splist_2.csv",'a') as F:
        F.write('\n')
        for a,b,c,d,e,f,g,h,i,j in zip(APG_Clade_list,
                     APG_Order_list,
                     APG_Family_list,
                     Kew_Family_list,
                     Genus_list,
...
for cal in range(maxduetime):
    ...
    if fine == amounte:
        ...
        Ltit=("All process is finsihed!! "+str(maxduetime)+"/"+str(maxduetime)+" "+str(fine/amounte*100))
        Lmain=(str(fine)+" of "+str(amounte)+" is finished.("+str(fine/amounte*100)+")\n "+str(rnum*maxduetime)+"URL is processed"
