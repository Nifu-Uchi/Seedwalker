...
def lister(urls):
    ...
    for linkid in linkslist:    
        ...
        htmllist.append("http://data.kew.org/sid/"+linkid)##idに必要文字列を連結してURLにして保存する
    ...
    names = bsObj.findAll("a",{"href":"SidServlet?ID=[A-Za-z0-9&=]+"})
    for name in names:
            nameslist.append((name.get_text()).replace(",","★"))
            print(name.get_text())
